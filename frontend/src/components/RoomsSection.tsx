  ...
  return (
    <section className="section" id="rooms">
      <div className="section-inner">
        ...
        >
          {rooms.map((r) => (
            <motion.article className="room" key={r.name} variants={item}>
              <div className="roomVisual" style={{ backgroundImage: `url(${r.image})` }} />
              <h3>{r.name}</h3>
              <div className="price">{r.price}<span style={{ fontSize: '0.8rem', fontWeight: 300, color: 'var(--muted)' }}> / nuit</span></div>
              <p style={{ padding: '8px 24px 0', fontSize: '0.9rem', color: 'var(--muted)', lineHeight: 1.7 }}>{r.description}</p>
              <ul>
                {r.features.map((f) => (
                  <li key={f}>{f}</li>
                ))}
